...
// Staff roles grouped by category
const ADMIN_ROLES: RoleDetail[] = [
  { 
    id: 'founder', 
    label: 'مؤسس', 
    icon: Crown, 
    color: 'amber', 
    bgColor: 'linear-gradient(135deg, #2b2d42, #121420)', 
  ...
  { 
    ...
    icon: Award, 
    color: 'rose', 
    bgColor: 'linear-gradient(135deg, #2b2d42, #1f2937)', 
  ...
  { 
    ...
    icon: UserCog, 
    color: 'sky', 
    bgColor: 'linear-gradient(135deg, #1e293b, #111827)', 
...
const TECHNICAL_ROLES: RoleDetail[] = [
  { 
    id: 'developer', 
    label: 'مطور', 
    icon: Code, 
    color: 'violet', 
    bgColor: 'linear-gradient(135deg, #1e1b4b, #0f172a)', 
  ...
  { 
    ...
    icon: Coffee, 
    color: 'fuchsia', 
    bgColor: 'linear-gradient(135deg, #4a044e, #1e1b4b)', 
  ...
  { 
    ...
    icon: Coffee, 
    color: 'indigo', 
    bgColor: 'linear-gradient(135deg, #312e81, #4a044e)', 
  ...
  { 
    ...
    icon: Settings, 
    color: 'teal', 
    bgColor: 'linear-gradient(135deg, #134e4a, #312e81)', 
...
// Combined roles for backward compatibility
const STAFF_ROLES = [...ADMIN_ROLES, ...TECHNICAL_ROLES];
...
const styles = {
  ...
  foundersGradient: {
    backgroundImage: 'linear-gradient(135deg, #2b2d42, #121420)'
  }
};
...
const AdminManagement = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    role: '',
    department: '',
    added_by: 'Admin', // Default value, should be replaced with the current admin
    username: '',
    password: '',
    fingerprint: '',
    can_login: true
  ...
  const [addedStaffData, setAddedStaffData] = useState<any | null>(null);
  const [copiedField, setCopiedField] = useState<string | null>(null);
  const staffDataRef = useRef<HTMLDivElement>(null);

  // Keep this for backward compatibility with existing UI
  ...
  // Fetch staff data with all roles
  ...
  const handleAddStaff = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setAddedStaffData(data.staff); // Store complete staff data
        
        // Set credentials data too for backward compatibility
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      // Make sure can_login is explicitly set as boolean
      const can_login_value = formData.can_login === true;
      ...
      const updates = {
        ...
        can_login: can_login_value  // Use the explicit boolean value
      };
      
      console.log("Sending updates:", JSON.stringify(updates)); // Debug log
  ...
  const resetForm = () => {
    ...
    setShowAddStaffForm(false);
    setShowDepartmentField(false);
    setGeneratedCredentials(null); // Reset credentials too
    // Don't reset addedStaffData here to keep it displayed
  };
  ...
  const getRoleDetails = (roleId: string): RoleDetail => {
    const adminRole = ADMIN_ROLES.find(r => r.id === roleId);
    if (adminRole) return adminRole;
    
    const techRole = TECHNICAL_ROLES.find(r => r.id === roleId);
    if (techRole) return techRole;
  ...
  // Copy all staff data as JSON
  const copyAllStaffData = () => {
    if (addedStaffData) {
      const dataStr = JSON.stringify(addedStaffData, null, 2);
        ...
        {addedStaffData && (
          ...
          >
            <div className="flex justify-between items-center mb-3">
              <h3 className="text-lg font-semibold text-purple-400">بيانات العضو الجديد</h3>
              <div className="flex gap-2">
                <button
                  onClick={copyAllStaffData}
                  className="px-3 py-1 bg-[#2d3748] text-white rounded-lg flex items-center gap-1 hover:bg-[#4a5568] transition-colors text-sm"
                >
                  {copiedField === 'all' ? (
                    <>
                ...
                <button
                  onClick={() => setAddedStaffData(null)}
                  className="p-1 bg-[#2d3748] text-gray-400 rounded-lg hover:bg-[#4a5568] transition-colors"
                >
                  <X size={16} />
                </button>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
              <div className="p-3 bg-[#2d3748] rounded-lg">
                <div className="flex justify-between">
                  <p className="text-gray-400 text-sm">معرف العضو:</p>
                  <button 
                    onClick={() => copyToClipboard(addedStaffData._id, '_id')}
                    className="text-gray-400 hover:text-white"
                    title="نسخ"
                  >
                    {copiedField === '_id' ? <CheckCircle size={14} className="text-green-400" /> : <Copy size={14} />}
                  </button>
                </div>
                <p className="text-white font-mono">{addedStaffData._id}</p>
              </div>
              ...
              <div className="p-3 bg-[#2d3748] rounded-lg">
                <div className="flex justify-between">
                  ...
                  <button 
                    onClick={() => copyToClipboard(addedStaffData.discord_id, 'discord_id')}
                    className="text-gray-400 hover:text-white"
                    title="نسخ"
                  >
                    {copiedField === 'discord_id' ? <CheckCircle size={14} className="text-green-400" /> : <Copy size={14} />}
                  </button>
                </div>
                <p className="text-white font-mono">{addedStaffData.discord_id}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
              <div className="p-3 bg-[#2d3748] rounded-lg">
                <div className="flex justify-between">
                  ...
                  <button 
                    onClick={() => copyToClipboard(addedStaffData.username, 'username')}
                    className="text-gray-400 hover:text-white"
                    title="نسخ"
                  >
                    {copiedField === 'username' ? <CheckCircle size={14} className="text-green-400" /> : <Copy size={14} />}
                  </button>
                </div>
                <p className="text-white font-mono">{addedStaffData.username}</p>
              </div>
              ...
              <div className="p-3 bg-[#2d3748] rounded-lg">
                <div className="flex justify-between">
                  ...
                  <button 
                    onClick={() => copyToClipboard(addedStaffData.password, 'password')}
                    className="text-gray-400 hover:text-white"
                    title="نسخ"
                  >
                    {copiedField === 'password' ? <CheckCircle size={14} className="text-green-400" /> : <Copy size={14} />}
                  </button>
                </div>
                <p className="text-white font-mono">{addedStaffData.password}</p>
              </div>
              ...
              <div className="p-3 bg-[#2d3748] rounded-lg">
                <div className="flex justify-between">
                  ...
                  <button 
                    onClick={() => copyToClipboard(addedStaffData.fingerprint, 'fingerprint')}
                    className="text-gray-400 hover:text-white"
                    title="نسخ"
                  >
                    {copiedField === 'fingerprint' ? <CheckCircle size={14} className="text-green-400" /> : <Copy size={14} />}
                  </button>
                </div>
                <p className="text-white font-mono truncate">{addedStaffData.fingerprint}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
              <div className="p-3 bg-[#2d3748] rounded-lg">
                <div className="flex justify-between">
                  <p className="text-gray-400 text-sm">اسم المستخدم على ديسكورد:</p>
                  <button 
                    onClick={() => copyToClipboard(addedStaffData.discord_username || '', 'discord_username')}
                    className="text-gray-400 hover:text-white"
                    title="نسخ"
                  >
                    {copiedField === 'discord_username' ? <CheckCircle size={14} className="text-green-400" /> : <Copy size={14} />}
                  </button>
                </div>
                <p className="text-white">{addedStaffData.discord_username || 'غير متوفر'}</p>
              </div>
              
              <div className="p-3 bg-[#2d3748] rounded-lg">
                <div className="flex justify-between">
                  <p className="text-gray-400 text-sm">البريد الالكتروني:</p>
                  <button 
                    onClick={() => copyToClipboard(addedStaffData.email || '', 'email')}
                    className="text-gray-400 hover:text-white"
                    title="نسخ"
                  >
                    {copiedField === 'email' ? <CheckCircle size={14} className="text-green-400" /> : <Copy size={14} />}
                  </button>
                </div>
                <p className="text-white">{addedStaffData.email || 'غير متوفر'}</p>
              </div>
              
              <div className="p-3 bg-[#2d3748] rounded-lg">
                <div className="flex justify-between">
                  <p className="text-gray-400 text-sm">الدور / المنصب:</p>
                  <button 
                    onClick={() => copyToClipboard(addedStaffData.role, 'role')}
                    className="text-gray-400 hover:text-white"
                    title="نسخ"
                  >
                    {copiedField === 'role' ? <CheckCircle size={14} className="text-green-400" /> : <Copy size={14} />}
                  </button>
                </div>
                <p className="text-white">{getRoleDetails(addedStaffData.role).label}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="p-3 bg-[#2d3748] rounded-lg">
                <div className="flex justify-between">
                  <p className="text-gray-400 text-sm">تاريخ الإضافة:</p>
                  <button 
                    onClick={() => copyToClipboard(addedStaffData.added_date, 'added_date')}
                    className="text-gray-400 hover:text-white"
                    title="نسخ"
                  >
                    {copiedField === 'added_date' ? <CheckCircle size={14} className="text-green-400" /> : <Copy size={14} />}
                  </button>
                </div>
                <p className="text-white">{formatDate(addedStaffData.added_date)}</p>
              </div>
              
              <div className="p-3 bg-[#2d3748] rounded-lg">
                <div className="flex justify-between">
                  <p className="text-gray-400 text-sm">تمت الإضافة بواسطة:</p>
                  <button 
                    onClick={() => copyToClipboard(addedStaffData.added_by, 'added_by')}
                    className="text-gray-400 hover:text-white"
                    title="نسخ"
                  >
                    {copiedField === 'added_by' ? <CheckCircle size={14} className="text-green-400" /> : <Copy size={14} />}
                  </button>
                </div>
                <p className="text-white">{addedStaffData.added_by}</p>
              </div>
            </div>

            <p className="mt-3 text-yellow-400 text-sm flex items-center gap-2">
              <b>هام:</b> قم بحفظ هذه البيانات، خاصة كلمة المرور حيث لن يتم عرضها مرة أخرى بهذا الشكل!
            </p>
          </motion.div>
        )}
        
        {/* Generated Credentials - Replace with the new Staff Data component */}
        {generatedCredentials && !addedStaffData && (
          <motion.div
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
            className="mb-6 p-4 bg-purple-500/20 border border-purple-500 rounded-lg"
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div>
                ...
                >
                  ...
                  <optgroup label="الأدوار الإدارية">
              ...
              {/* Department field for managers */}
              {showDepartmentField && (
                <div className="md:col-span-2">
                  <label className="block text-gray-400 mb-2">القسم</label>
                  <input
                    type="text"
                    name="department"
                    ...
                    onChange={handleFormChange}
                    className="w-full bg-[#1e293b] text-white p-3 rounded-lg border border-[#4a5568] focus:outline-none focus:ring-2 focus:ring-purple-500"
                    placeholder="أدخل اسم القسم الذي يديره المسؤول"
                  />
                </div>
              )}

              {/* Add can_login field for adding new staff (not just editing) */}
              {!editingStaff && (
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    <label className="flex items-center cursor-pointer">
                      <input
                        ...
                        onChange={() => {
                          console.log("Setting can_login to true");
                    ...
                    <label className="flex items-center cursor-pointer">
                      <input
                        ...
                        onChange={() => {
                          console.log("Setting can_login to false");
                          setFormData({...formData, can_login: false});
                        }}
                        className="h-4 w-4 text-purple-600"
                      />
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                disabled={isLoading || !formData.discord_id || !formData.role || (showDepartmentField && !formData.department)}
          ...
          <>
            {isLoading && staffMembers.length === 0 ? (
              ...
              <div className="bg-[#2d3748] rounded-lg p-8 text-center">
                ...
                <h3 className="text-white text-lg font-medium mb-1">لا يوجد موظفين</h3>
              ...
              <div className="space-y-8">
                ...
                {(() => {
                  ...
                  // الأدوار حسب ترتيبها
                  const sortedRoles = [...ADMIN_ROLES, ...TECHNICAL_ROLES]
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="relative">
                                  ...
                                  <div className="absolute -bottom-10 left-4 w-20 h-20 rounded-full overflow-hidden shadow-lg">
                                    <div 
                                      className={`absolute inset-0 border-4 rounded-full ${
                                        staff.role === 'founder' ? 'border-amber-500/70' : 
                                        staff.role === 'general_manager' ? 'border-rose-500/70' : 
                                        'border-[#2d3748]'
                                      }`}
                                    ></div>
                                    <img
                                  ...
                                  <div
                                    className={`absolute top-4 right-4 px-3 py-1.5 rounded-full text-xs font-medium flex items-center gap-1 bg-[#0f1117]/70 backdrop-blur-sm border ${
                                      staff.role === 'founder' ? 'border-amber-500/40' : 
                                      staff.role === 'general_manager' ? 'border-rose-500/40' : 
                                      roleDetails.borderColor?.replace('border-', 'border-') || 'border-gray-600/40'
                                    }`}
                                  >
                                ...
                                <div className="pt-12 pb-4 px-4">
                                  ...
                                  {staff.department && staff.role === 'manager' && (
                                    <div className="mb-3 p-2 bg-[#14162a] rounded-lg shadow-inner">
